...
const pluginRoot = resolve(process.env.CLAUDE_PLUGIN_ROOT || scriptPluginRoot);
...
function statePaths(sessionId) {
  const dataRoot = process.env.CLAUDE_PLUGIN_DATA?.trim();
...
export async function runHook(mode, rawInput = "") {
  ...
  if (!existsSync(pluginRoot)) {
    return hookOutput(mode, "ADAPTIVE_ROUTER_FOR_CLAUDE: CLAUDE_PLUGIN_ROOT is unavailable. Continue with the current model.");
  }
  if (mode === "SessionStart") return hookOutput("SessionStart", renderSessionContext());
  if (!rawInput.trim()) {
    return hookOutput("UserPromptSubmit", "ADAPTIVE_ROUTER_FOR_CLAUDE: No prompt payload was available. Continue with the current model.");
  }
  ...
  try {
    ...
    return hookOutput("UserPromptSubmit", "ADAPTIVE_ROUTER_FOR_CLAUDE: The hook payload could not be parsed. Continue with the current model.");
  }
