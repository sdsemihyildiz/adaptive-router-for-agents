...
export function decideRoute({ prompt = "", activeModel = "", previousState = null } = {}) {
  ...
  const direct = route === "adaptive_haiku" && activeModel === target.model;
  return {
    route,
    model: target.model,
...
export function renderDecisionContext(decision) {
  ...
  return `ADAPTIVE_ROUTER_FOR_CLAUDE
ROUTE=${decision.route}
...
- If DIRECT=false, call the adaptive-router-for-claude MCP tool run_routed_task exactly once from the root task with route ${decision.route}, a complete task brief, the current working directory, and the current safe sandbox level.
