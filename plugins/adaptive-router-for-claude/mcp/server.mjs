...
export function createServer(runWorkerImpl = runWorker) {
  const server = new McpServer({ name: "adaptive-router-for-claude", version: "0.1.0" });
  server.registerTool(
    "run_routed_task",
    {
      title: "Run task with selected Claude model tier",
      description: "Run a complete task through the model and reasoning tier selected by Adaptive Router for Claude Code. Invoke it exactly once from the root task for normal non-direct routes; never invoke it from a subagent.",
      inputSchema: {
        ...
        cwd: z.string().optional().describe("Absolute working directory. Defaults to the current Claude Code working directory."),
